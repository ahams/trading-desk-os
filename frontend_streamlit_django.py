"""
Trading Desk OS - Beta Frontend (Django JWT)
---------------------------------------------
Streamlit UI for the TDOS Django control plane.

Authentication flow:
    Streamlit -> Django JWT -> Django quota/subscription -> FastAPI analytics

Run:
    streamlit run frontend_streamlit_django.py

Django endpoints used:
    POST /api/auth/login/
    POST /api/auth/refresh/
    GET  /api/account/
    POST /api/analysis/compact/
    POST /api/scanner/compact/   (requires Django scanner proxy)
    POST /api/report/daily/      (requires Django report proxy)

Environment variables supported:
    TDOS_API_URL   # Django base URL, default http://127.0.0.1:9000
"""
...
DEFAULT_API_URL = os.getenv(
    "TDOS_API_URL",
    os.getenv("DJANGO_API_URL", "http://127.0.0.1:9000"),
)
...
def login_user(username: str, password: str) -> Tuple[bool, Any]:
    ...
    if not base_url:
        return False, "Missing Django API URL"
    ...
    try:
        ...
        if not isinstance(body, dict) or not body.get("access"):
            return False, "Django login response did not contain an access token."

        st.session_state.access_token = body["access"]
        st.session_state.refresh_token = body.get("refresh", "")
        st.session_state.authenticated_username = username
        return True, body

    except requests.exceptions.ConnectionError:
        return False, f"Could not connect to Django at {base_url}. Is Django running?"
    except requests.exceptions.Timeout:
        return False, f"Django login timed out: {base_url}"
    except Exception as exc:
        return False, f"Unexpected login error: {exc}"


def refresh_access_token() -> Tuple[bool, Any]:
    refresh = st.session_state.get("refresh_token", "")
    if not refresh:
        return False, "No refresh token available. Please sign in again."
    ...
    try:
        ...
        if not isinstance(body, dict) or not body.get("access"):
            _clear_auth_state()
            return False, "Refresh response did not contain an access token."

        st.session_state.access_token = body["access"]
        # ROTATE_REFRESH_TOKENS may return a replacement refresh token.
        if body.get("refresh"):
            st.session_state.refresh_token = body["refresh"]
        return True, body
...
def api_request(
    ...
    *,
    ...
    """Call the Django control plane with JWT auth.

    If an access token expires, refresh it once and transparently retry the
    original request. The browser never needs the FastAPI internal key.
    """
    base_url = normalize_base_url(st.session_state.get("api_url", DEFAULT_API_URL))
    if not base_url:
        return False, "Missing Django API URL"
    ...
    try:
        ...
        if resp.status_code == 401 and retry_auth and st.session_state.get("refresh_token"):
            refreshed, refresh_resp = refresh_access_token()
            if refreshed:
                return api_request(
                    method,
                    path,
        ...
        return False, f"Could not connect to Django API at {url}. Is Django running?"
    except requests.exceptions.Timeout:
...
def fetch_account() -> Tuple[bool, Any]:
    ok, resp = api_request("GET", "/api/account/", timeout=30)
    if ok and isinstance(resp, dict):
        st.session_state.account_data = resp
    return ok, resp


def logout_user() -> None:
    _clear_auth_state()
    # Clear data belonging to the previous authenticated session.
    for key in (
        "analysis_data",
        "analysis_raw_response",
        "analysis_data_ticker",
        "scanner_df",
...
def show_proxy_hint(err: Any, feature: str) -> None:
    """Explain the expected next backend step for scanner/report proxy 404s."""
    if isinstance(err, dict) and err.get("status_code") == 404:
        st.error(f"{feature} Django proxy is not installed yet.")
        st.info(
            f"The frontend is correctly using Django JWT. Add the Django {feature.lower()} "
            "proxy endpoint, then retry. Do not re-enable the old X-API-Key flow."
        )
        return
    show_error(err)
...
def _find_witness(data, *engine_names):
    ...
    # ------------------------------------------------------------------
    # 1. Preferred source: full committee witnesses
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 2. New committee representation: decision_drivers
    # ------------------------------------------------------------------
    drivers = reasoning.get("decision_drivers") or []

    for driver in drivers:
        if not isinstance(driver, dict):
            continue

        if str(driver.get("engine", "")).lower() in wanted:
            return driver
    ...
    if isinstance(decisive, dict):
        if str(decisive.get("engine", "")).lower() in wanted:
            return decisive

    # ------------------------------------------------------------------
    # 4. Other committee evidence buckets
    # ------------------------------------------------------------------
    for bucket_name in (
        "supporting_evidence",
        ...
        for item in reasoning.get(bucket_name) or []:
            if not isinstance(item, dict):
                continue

            if str(item.get("engine", "")).lower() in wanted:
                return item
...
def _render_witness_header(
    ...
    # If there is genuinely no committee engine record,
    # don't pretend the direction is neutral.
    direction_display = (
        str(direction).title()
        if direction
        else "—"
...
def render_expectation_explainer(data):
    ...
    _render_witness_header(
        witness,
        scores.get("expectation"),
        show_claim=False,
    )
    ...
    growth_gap = _first_value(
        snap,
        "growth_gap_pct",
        "growth_gap",
    )

    # Calculate presentation-only gap when backend did not return it.
    if growth_gap is None:
        try:
            if revenue_growth is not None and implied_cagr is not None:
                growth_gap = float(revenue_growth) - float(implied_cagr)
    ...
    c1.metric(
        "Market-implied CAGR",
        _metric_or_dash(implied_cagr, "%"),
    )

    c2.metric(
        "Revenue Growth",
        _metric_or_dash(revenue_growth, "%"),
    )

    c3.metric(
        "CAP Years",
        _metric_or_dash(cap_years),
    )
    ...
    c1.metric(
        "ROIC",
        _metric_or_dash(roic, "%"),
    )

    c2.metric(
        "Market-required MEROI",
        _metric_or_dash(meroi, "%"),
    )

    c3.metric(
        "Growth Gap",
        _metric_or_dash(growth_gap, "%"),
    )
    ...
    read = (
        snap.get("read")
        or (data.get("reads") or {}).get("expectation")
    )
    ...
    supporting, drawbacks, invalidators = (
        _classify_expectation_evidence(witness)
    )
    ...
    with c2:
        _render_evidence_list(
            "Current drawbacks / valuation hurdles",
            drawbacks,
            "warning",
        )
...
st.sidebar.caption("Django-authenticated beta frontend")
...
if not is_authenticated:
    ...
    with st.sidebar.form("tdos_login_form", clear_on_submit=False):
    ...
    if login_submit:
        if not login_username.strip() or not login_password:
            ...
            ok, resp = login_user(login_username.strip(), login_password)
            if ok:
                account_ok, _ = fetch_account()
                if account_ok:
                    st.sidebar.success("Signed in")
                else:
                    st.sidebar.warning("Signed in, but account details could not be loaded.")
                st.rerun()
            else:
                st.sidebar.error("Sign in failed")
                st.sidebar.write(resp)
else:
    account = st.session_state.get("account_data") or {}
    ...
    display_name = (
        ...
        or st.session_state.get("authenticated_username")
        or "TDOS user"
    )
    ...
    c1.metric("Plan", subscription.get("plan", "n/a"))
    c2.metric("Remaining", usage.get("remaining", "n/a"))

    if st.sidebar.button("Refresh Account", use_container_width=True):
        ok, resp = fetch_account()
        if ok:
            st.sidebar.success("Account refreshed")
            st.rerun()
        else:
            st.sidebar.error("Could not refresh account")
            st.sidebar.write(resp)
...
st.sidebar.divider()
st.sidebar.caption("Authentication: Django JWT. FastAPI service credentials stay server-side.")
...
if not st.session_state.get("access_token"):
    st.info("Sign in from the sidebar to access TDOS.")
    st.stop()

# Load account metadata once after login/session restore.
if not st.session_state.get("account_data"):
    account_ok, account_resp = fetch_account()
    if not account_ok:
        st.warning("Authenticated, but account details could not be loaded.")
...
if nav_page == "Account":
    ...
    if isinstance(account, dict):
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("User", user.get("username", "n/a"))
        c2.metric("Plan", subscription.get("plan", "n/a"))
        c3.metric("Usage", usage.get("used", "n/a"))
        c4.metric("Remaining", usage.get("remaining", "n/a"))

        c1, c2, c3 = st.columns(3)
        c1.metric("Monthly Limit", subscription.get("monthly_analysis_limit", usage.get("limit", "n/a")))
        c2.metric("Account Active", "Yes" if user.get("is_active") else "No")
        c3.metric("Subscription Active", "Yes" if subscription.get("active") else "No")

        if user.get("email"):
            st.caption(f"Email: {user.get('email')}")
        if subscription.get("valid_until"):
            st.caption(f"Subscription valid until: {subscription.get('valid_until')}")

        with st.expander("Account API response", expanded=False):
            st.json(account)
    ...
    if manual_analyze or scanner_analyze:
        if not ticker:
            ...
            if ok:
                ...
                # Django appends fresh quota metadata after a successful analysis.
                account_meta = resp.get("_account") if isinstance(resp, dict) else None
                if isinstance(account_meta, dict):
                    account_state = st.session_state.get("account_data") or {}
                    if isinstance(account_state, dict):
                        usage = account_state.setdefault("usage", {})
                        usage["used"] = account_meta.get("usage_used", usage.get("used"))
                        usage["limit"] = account_meta.get("usage_limit", usage.get("limit"))
                        usage["remaining"] = account_meta.get("usage_remaining", usage.get("remaining"))
                        user_state = account_state.setdefault("user", {})
                        subscription_state = user_state.setdefault("subscription", {})
                        subscription_state["plan"] = account_meta.get("plan", subscription_state.get("plan"))
                        st.session_state.account_data = account_state
            else:
                show_error(resp)
...
if nav_page == "Daily Report":
    ...
    if st.button("Generate Daily Report", type="primary", use_container_width=True):
        ...
        if ok:
            ...
            show_proxy_hint(resp, "Daily Report")
# =============================================================================
